...
<?php
	...
	if (isset($_POST['register'])) {
		...
		if ($password==$kPassword) {
			...
			$wallet=mysql_query("INSERT INTO `tb_wallet` (`id_wallet`, `id_user`) VALUES ('$idWallet','$idUser');");
			if ($insert&&$wallet) { ?>
	...
	if (isset($_POST['updatePasword'])) {
		...
		if ($password==$kPassword) {
			...
			if ($update) { ?>
				<script type="text/javascript">
					alertify.alert("Password Gagal Di Ubah", function(){window.location.assign('../profil')}).setHeader(' ').set({closable:false,transition:'fade'});
				</script>
			<?php
			}else{?>
                <script language="JavaScript">
                    alertify.alert("Password Gagal Di Ubah", function(){ window.location.assign('../profil'); }).setHeader(' ').set({closable:false,transition:'pulse'});
               </script>
            <?php
            }
		}else{?>
           <script language="JavaScript">
                alertify.alert("Password Tidak Sesuai", function(){ window.location.assign('../profil'); }).setHeader(' ').set({closable:false,transition:'pulse'});
           </script>
        <?php
        }
	}
	...
	if (isset($_POST['update'])) {
		...
		if(in_array($ekstensi, $ekstensi_diperbolehkan) === true && in_array($ekstensi1, $ekstensi_diperbolehkan) === true){
			if (($ukuran<=$sizeByte && $ukuran!=0) && ($ukuran1<=$sizeByte && $ukuran1!=0)) {
				move_uploaded_file($file_tmp, '../assets/images/user/'.$namaBaru);
				move_uploaded_file($file_tmp1, '../assets/images/user/'.$namaBaru1);
				...
				if ($update) { ?>
					<script type="text/javascript">
						alertify.alert("Data Telah Di Ubah", function(){window.location.assign('../profil')}).setHeader(' ').set({closable:false,transition:'fade'});
					</script>
				<?php
				}else{?>
                    <script language="JavaScript">
                        alertify.alert("Gagal Upload", function(){ window.location.assign('../profil'); }).setHeader(' ').set({closable:false,transition:'fade'});
                    </script>
                <?php
                }
			}else{?>
                <script language="JavaScript">
                    alertify.alert("Ukuran Tidak Sesuai, Ukuran Max Adalah 1MB", function(){ window.location.assign('../profil'); }).setHeader(' ').set({closable:false,transition:'fade'});
                </script>
            <?php
            }
		}else{?>
	...
	if (isset($_POST['resetPasword'])) {
		...
		if ($password==$kPassword) {
			...
			if ($update) { ?>
				<script type="text/javascript">
					alertify.alert("Password Berhasil Di Ubah", function(){window.location.assign('../profil')}).setHeader(' ').set({closable:false,transition:'fade'});
				</script>
			<?php
			}else{?>
                <script language="JavaScript">
