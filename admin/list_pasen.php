    ...
    <body class="fixed-left">
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                <div class="page-title-box">
                                    <h4 class="page-title">List PASEN</h4>
                                ...
                                <div class="card-box card-scroll-2 scroll-xy">
                                    <table id="datatable-buttons" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                            <?php 
                                                $no=1;
                                                $dataDesain=mysql_query("SELECT * FROM tb_desain JOIN tb_kategori ON tb_desain.id_kategori=tb_kategori.id_kategori WHERE pemilik in (SELECT id_user FROM tb_user)");
                                                while ($dataTables=mysql_fetch_array($dataDesain)) { 
                                                    $status=$dataTables['status'];
                                                ?>
                                                <tr>
                                                    <td><?=$no;?></td>
                                                    <td><?=$dataTables['id_desain'];?></td>
                                                    <td><?=$dataTables['nama_desain'];?></td>
                                                    <td><?=rupiah($dataTables['harga']);?></td>
                                                    <td><?=tanggal($dataTables['tgl_upload']);?></td>
                                                    <td><?=$dataTables['nama_kategori'];?></td>
                                                    <td><a href="../preview_desain?id=<?=$dataTables['id_desain'];?>" class="txt-look" target='blank'>Lihat PASEN</a></td>
                                                    <td>
                                                        <select onchange="ubahStatus(this.value,'<?=$dataTables['id_desain'];?>')">
        ...
        <script type="text/javascript">
            function ubahStatus(status,idArtikel){
                window.location.assign('php/proses_pasen.php?st='+status+'&id='+idArtikel);
            }
