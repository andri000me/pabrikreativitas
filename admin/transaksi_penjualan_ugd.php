...
<html>
    <head>
        ...
        <!-- App title -->
        <title>List Transaksi Desain (UDG)</title>
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                <div class="page-title-box">
                                    <h4 class="page-title">List Transaksi Desain (UDG)</h4>
                                ...
                                <div class="card-box card-scroll-2 scroll-xy">                              
                                    <table id="datatable-buttons" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                            <?php 
                                                $no=1;
                                                $dataTransaksi=mysql_query("SELECT tb_transaksi.no_invoice, tb_transaksi.status, nama_desain, tgl_transaksi, sub_total FROM tb_transaksi JOIN tb_detail_transaksi ON tb_transaksi.no_invoice=tb_detail_transaksi.no_invoice JOIN tb_desain ON tb_detail_transaksi.id_item=tb_desain.id_desain JOIN tb_user ON tb_transaksi.id_user=tb_user.id_user  WHERE SUBSTR(tb_transaksi.no_invoice,5,1)='D' AND pemilik NOT IN (SELECT id_user FROM tb_user)") or die(mysql_error());
                                                while ($dataTables=mysql_fetch_array($dataTransaksi)) {
                                                    if (strlen($dataTables['nama_desain'])>12) {
                                                        $namaDesain=str_pad(substr($dataTables['nama_desain'],0,12),15,".");
                                                    }else{
                                                        $namaDesain=$dataTables['nama_desain'];
                                                    }
