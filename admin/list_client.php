    ...
    <body class="fixed-left">
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                <div class="page-title-box">
                                    <h4 class="page-title">List Client <small>Kerja Sama</small></h4>
                                ...
                                <div class="card-box card-scroll-2 scroll-xy">
                                    <table id="datatable-buttons" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                            <?php 
                                                $no=1;
                                                $dataClient=mysql_query("SELECT * FROM tb_client");
                                                while ($dataTables=mysql_fetch_array($dataClient)) { ?>
                                                <tr>
                                                    <td><?=$no;?></td>
                                                    <td><?=$dataTables['id_client'];?></td>
                                                    <td><?=$dataTables['nama_client'];?></td>
                                                    <td><?=$dataTables['alamat'];?></td>
                                                    <td><?=$dataTables['pemilik'];?></td>
                                                    <td><?=$dataTables['no_hp'];?></td>
                                                    <td><?=tanggal($dataTables['tgl_mou']);?></td>
                                                    <td><a href="edit_client?id=<?=$dataTables['id_client'];?>" class="txt-edit">Edit</a>&nbsp;<a href="javascript:hapusDesain('<?=$dataTables['id_client'];?>')" class="txt-hapus">Hapus</a></td>
                                                </tr>
                                                <?php
                                                $no++;
                                                }
        ...
        <script type="text/javascript">
            function hapusDesain(id){
                alertify.confirm('Perhatian', 'Anda Yakin Akan Menghapus '+id, function(){ window.location.assign('php/proses_client?hps='+id) }
