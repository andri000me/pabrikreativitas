...
<html lang="en">
    ...
    if ($getK!='all') { 
        $kb=$_GET['k'];
        $qDesain = mysql_query("SELECT * FROM tb_desain WHERE id_kategori='$kb' AND status='1' LIMIT $mulai, $halaman")or die(mysql_error); 
        $result = mysql_query("SELECT id_desain  WHERE id_kategori='$kb' AND status='1' FROM tb_desain");
    } else { 
        $kb='all'; 
        $qDesain = mysql_query("SELECT * FROM tb_desain WHERE status='1' LIMIT $mulai, $halaman")or die(mysql_error);
        $result = mysql_query("SELECT id_desain FROM tb_desain WHERE status='1' ");
    }
                    ...
                    <div class="col-lg-2">  
                        ...
                        <div class="card c-t-1 c-b-2">
                            <div class="card-container">
                                ...
                                <button class="btn btn-sm bg-btn-dark" style="width: 100%"; onclick="window.location.assign('pasen?page=1&k=all')">Tampilkan Semua</button>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-10">         
                                ...
                                <div class="form-inline">
                                    <div class="row">
                                        ...
                                        while ($desain=mysql_fetch_array($qDesain)) {
                                            ...
                                            $dPemilik=substr($desain['pemilik'], 0,2);
                                            if ($dPemilik!='AD') {
                                                $idPemilik=$desain['pemilik'];
                                                $ambilNama=mysql_fetch_array(mysql_query("SELECT nama_user FROM tb_user WHERE id_user='$idPemilik'"));
                                                $pemilik=$ambilNama['nama_user'];
                                            }else{
                                                $pemilik='Official Product';
                                            }
                                        ?>
                                                ...
                                                <div class="form-group card card-cursor" onclick="detail('<?=$idDesain;?>')">
                                                    ...
                                                    <div class="card-block">
                                                        <div class="card-name">
                                                            ...
                                                            <p class="card-title text-left"><b><?=$pemilik;?></b></p>
                                                        </div>                                      
                                                    </div>
                                                </div>
                                            </div>
